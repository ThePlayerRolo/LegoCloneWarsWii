...
inline static ISFSError nandDelete(const char *path, NANDCommandBlock *block,
...
inline static ISFSError nandSeek(const IOSFd fd, const s32 offset, const s32 whence,
...
inline s32 NANDPrivateCreateDir(const char *path, u8 perm, u8 attr) {
  if (!nandIsInitialized()) {
    return NAND_RESULT_FATAL_ERROR;
  }
...
inline static ISFSError nandGetFileStatus(IOSFd fd, u32 *length, u32 *pos) {
...
inline s32 NANDGetLength(NANDFileInfo *info, u32 *length) {
  if (!nandIsInitialized()) {
    return NAND_RESULT_FATAL_ERROR;
  }
...
inline void nandGetFileStatusAsyncCallback(ISFSError result, void *ctxt) {
...
inline static ISFSError nandGetFileStatusAsync(IOSFd fd, NANDCommandBlock *block) {
...
inline s32 NANDGetLengthAsync(NANDFileInfo *info, u32 *length, NANDCallback cb,
                       NANDCommandBlock *block) {
  if (!nandIsInitialized()) {
    return NAND_RESULT_FATAL_ERROR;
  }
...
//This is being inlined for some reason?
void nandComposePerm(u8 *perm, const u32 ownerAcc, const u32 groupAcc,
                     const u32 othersAcc) DECOMP_DONT_INLINE {
  u32 p = 0;
...
inline s32 NANDGetStatus(const char *path, NANDStatus *stat) {
  if (!nandIsInitialized()) {
    return NAND_RESULT_FATAL_ERROR;
  }
...
inline void NANDSetUserData(NANDCommandBlock *block, void *data) {
  block->userData = data;
}

inline void *NANDGetUserData(const NANDCommandBlock *block) { return block->userData; }
