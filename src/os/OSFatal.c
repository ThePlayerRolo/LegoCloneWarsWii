...
void OSFatal(GXColor textColor, GXColor bgColor, const char* msg) {
    ...
    OSSetMEM1ArenaLo((void*)0x81400000);
    if(bootInfo->FSTLocation == NULL) OSSetMEM1ArenaHi(*(void**)OSPhysicalToCached(OS_PHYS_USABLE_MEM1_END));
    else OSSetMEM1ArenaHi(bootInfo->FSTLocation);

    FatalParam.textColor = textColor;
    FatalParam.bgColor = bgColor;
    FatalParam.msg = msg;
    OSSwitchFiber(Halt, OSGetMEM1ArenaHi());
}
...
static void Halt(void) {
    ...
    OSLoadFont(msgFont, OSGetMEM1ArenaLo());
